...
public class MyAdapter extends RecyclerView.Adapter<MyAdapter.ViewHolder> {
    ...
    //
    public MyAdapter(List < dog > values, Context context) {
        this.values = values;
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        final dog dogm = values.get(position);
        holder.txtHeader.setText("ชื่อสายพันธ์ : "+dogm.getDog_name());
        holder.txtFooter.setText(dogm.getDog_type());

        //ใช้ Glide ในการแสดงภาพบน ImageView  ซึ่งอาจจะใช้ Picasso แทนก็ได้
        //
        Glide.with(context)
                .load(dogm.getDog_image())
                .into(holder.imgView);
